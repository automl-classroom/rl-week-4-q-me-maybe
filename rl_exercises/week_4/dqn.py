...
class DQNAgent(AbstractAgent):
    ...
    def predict_action(
        ...
        if evaluate:
            ...
            with torch.no_grad():
                qvals = self.q(state_tensor)  # noqa: F841
    ...
    def update_agent(
        ...
        with torch.no_grad():
            max_q_next = self.target_q(s_next).max(1)[0].unsqueeze(1)
            print(max_q_next)
            print(r.unsqueeze(1))
            target = r.unsqueeze(1) + self.gamma * max_q_next * (1 - mask.unsqueeze(1))
        ...
        for frame in range(1, num_frames + 1):
            ...
            if done or truncated:
                ...
                if len(recent_rewards) % 10 == 0:
                    ...
                    eval_interval = min(eval_interval, len(recent_rewards))
...
def main(cfg: DictConfig):
    ...
    agent.train(num_frames=cfg.total_timesteps, eval_interval=cfg.eval_interval)
